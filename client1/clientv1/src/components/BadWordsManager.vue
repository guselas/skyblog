      ...
      errorMsg
        ...
        <div>
            <b-modal @ok="putBadword" id="edit-badword" title="Edit badword">
                <b-form>
                    ...
                    <b-form-group>
                        ...
                        <b-input v-model="currentLevel" type="number" min="1" max="5"></b-input>
                    </b-form-group>
                </b-form>
            </b-modal>
            <b-modal @ok="deleteBadword" id="delete-badword" title="Delete badword">
                <b-form>
                    <b-form-group>
                        <label for="">Word</label>
                        <b-input v-model="currentWord" type="text" readonly="readonly"></b-input>
                    </b-form-group>
                    <b-form-group>
                        <label for="">Level</label>
                        <b-input v-model="currentLevel" type="number" min="1" max="5" readonly="readonly"></b-input>
                    </b-form-group>
                </b-form>
            </b-modal>
        </div>
        ...
        <table class="table table-striped ">
            <thead>
                <tr class="d-flex">
                    <th class="col-5">
                        <input type="text" v-model="newBadword" placeholder="BadWord">
                        <label for="newLevel">Level</label>
                        <input type="number" v-model="newLevel" min="1" max="5">

                        <button class="btn btn-sm btn-secondary" @click="addBadword">Add</button>
                    </th>
                    <th class="col-5">
                        word
                    </th>
            ...
            <tbody>
                <tr class="d-flex" v-for="(badword,index) in badwords" :key="index">
                    <td class="col-5">
                        <b-button @click="showBadwordForEdit(index)" class="btn btn-sm btn-primary">Edit
                        </b-button>
                        <b-button @click="showBadwordForDelete(index)" class="btn btn-sm btn-danger">
                            Delete</b-button>
        ...
        methods: {
            ...
            async putBadword() {
                try {
                    ...
                    await axios.put(`http://localhost:3000/api/blog/badwords/${badwordDTO.id}`, badwordDTO);
                    await this.getBadwords();
            ...
            async deleteBadword() {
                try {
                    await axios.delete(`http://localhost:3000/api/blog/badwords/${this.currentId}`);
                    await this.getBadwords();

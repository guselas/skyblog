<template>
  <div>
    <div class="container">
      ...
      <b-modal ref="my-editPost" id="modal-edit" title="Edit Post" size="lg" @hidden="resetModal" @ok="handleOk"
        @show="resetModal">
        ...
        <h3>Post Category</h3>
        <br>
        <b-form-textarea id="textarea-postCategory" v-model="category" :state="nameState" required></b-form-textarea>
        <p>{{ postDateUpdate }}</p>
        <p>{{ postLastUpdate }}</p>
      ...
      <div v-if="myPostsLength">
        ...
        <div v-for="(post, index) in myPosts" :key="post.Id">
          <div class="card">
            <div class="card-header" style="text-align:center;">{{ post.postTitle }} // {{ index }}</div>
            <div class="card-body">
              {{ post.postText }}
            </div>
              ...
              <div>
                <b-form inline>
                  <p style="text-align:center">
                    <b-badge variant="primary"> NickName: </b-badge>{{ post.nickName }}
                  </p>
                  <hr />
                  ...
                  <hr />
                  <b-button v-b-modal.modal-delete prevent-closing variant="danger" @click="deletePost()">Delete
                  </b-button>
                  <b-button v-b-modal.modal-edit prevent-closing @click="loadPostInfo(index)">Edit</b-button>
                </b-form>
              </div>
            </div>
          </div>
